...
def load_and_transform_data(args):
    ...
    # 🔧 SỬA DÒNG NÀY: dùng điều kiện linh hoạt hơn thay vì "if dataset == 'CN'"
    if hasattr(args.dataset, "tmp") and hasattr(args.dataset, "aqi"):
    #if args.dataset.dataset == "CN": # dòng gốc với chỉ riêng Trung Quốc 
    ...
    encoder = OneHotEncoder(sparse_output=False) # phiên bản mới của skit learn đã thay đổi 
    ...
    # 🔧 SỬA DÒNG NÀY: tạo mảng `data` theo từng trường hợp có hoặc không có tmp/aqi
    if tmp_np is not None and aqi_np is not None:
    #if args.dataset.dataset == "CN": # dòng gốc với chỉ riêng Trung Quốc
        data = np.concatenate(
            (
